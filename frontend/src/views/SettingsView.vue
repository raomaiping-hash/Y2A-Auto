...
const SECTIONS: SectionDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'SUBTITLE_QC_BASE_URL', label: '质检独立地址', type: 'text', hint: '留空回退字幕翻译/主地址' },
      { key: 'SUBTITLE_QC_API_KEY', label: '质检独立 Key', type: 'password', sensitive: true, hint: '留空回退字幕翻译/主 Key' },
      ...
      { key: 'SUBTITLE_PREFER_SINGLE_LINE', label: '优先单行显示', type: 'toggle', hint: '优先整句单行，放不下再折行' },
      { key: 'SUBTITLE_SINGLE_LINE_MIN_FONT_SCALE', label: '单行最小字号缩放', type: 'number', step: '0.05', hint: '单行放不下时缩小字号的下限比例' },
    ],
  },
  {
    id: 'subtitle_style',
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'VAD_PROVIDER', label: 'VAD 引擎', type: 'text', placeholder: 'silero-vad' },
      { key: 'VAD_REFINEMENT_ENABLED', label: 'VAD 边界二次收敛', type: 'toggle', hint: '对粗检出的语音窗执行二次边界收敛提升精度' },
    ],
  },
  {
    id: 'ai_segmentation',
    title: 'AI 智能分段',
    icon: 'bi-scissors',
    desc: '基于字级时间戳的语义重分段（实验性，默认关闭；需 ASR 支持字级时间戳）',
    fields: [
      { key: 'AI_SEGMENTATION_ENABLED', label: '启用 AI 智能分段', type: 'toggle', hint: '需 ASR 支持字级时间戳，否则自动降级' },
      { key: 'AI_SEGMENTATION_MODEL_NAME', label: '分段模型', type: 'text', hint: '留空回退主模型' },
      { key: 'AI_SEGMENTATION_BASE_URL', label: '分段独立地址', type: 'text', hint: '留空回退主地址' },
      { key: 'AI_SEGMENTATION_API_KEY', label: '分段独立 Key', type: 'password', sensitive: true },
      { key: 'AI_SEGMENTATION_THINKING_ENABLED', label: '分段思考模式', type: 'toggle' },
      { key: 'AI_SEGMENTATION_MIN_CUE_DURATION_S', label: '最短段长（秒）', type: 'number', step: '0.5' },
      { key: 'AI_SEGMENTATION_MAX_CUE_DURATION_S', label: '最长段长（秒）', type: 'number', step: '0.5' },
      { key: 'AI_SEGMENTATION_MAX_CPS', label: '最大每秒字符数', type: 'number', step: '0.5' },
      ...
      { key: 'AI_SEGMENTATION_CONTEXT_WINDOW', label: '上下文窗口', type: 'number' },
    ],
  },
  {
    id: 'dubbing',
    title: '配音',
    icon: 'bi-mic-fill',
    desc: 'Fish Audio 语音合成：中文字幕配音（完全去掉原声，可选 BGM 垫底）',
    fields: [
      { key: 'DUBBING_ENABLED', label: '启用配音', type: 'toggle', hint: '开启后任务流水线自动配音（任务/监控可单独覆盖）' },
      { key: 'DUBBING_VOICE_ID', label: '默认音色 ID', type: 'text', hint: 'Fish Audio 预置音色 reference_id，任务/监控未设置时使用' },
      { key: 'FISH_API_KEY', label: 'Fish API Key', type: 'password', sensitive: true, hint: 'https://fish.audio/app/developers 获取' },
      { key: 'FISH_TTS_MODEL', label: '合成模型', type: 'select', options: [{ value: 's2.1-pro-free', label: 's2.1-pro-free（免费）' }, { value: 's2.1-pro', label: 's2.1-pro（付费，需 API 余额）' }] },
      { key: 'FISH_TTS_TIMEOUT_SECONDS', label: '合成超时（秒）', type: 'number' },
      { key: 'FISH_REQUEST_INTERVAL_SECONDS', label: '请求间隔（秒）', type: 'number', step: '0.05', hint: '串行防限流，建议 ≥0.3' },
      { key: 'DUBBING_OPTIMIZE_SCRIPT_ENABLED', label: '配音文案 AI 优化', type: 'toggle', hint: '超长句用 AI 压缩（意思不变），使配音更自然' },
      { key: 'DUBBING_ALIGN_OVERFLOW_THRESHOLD', label: '对齐溢出阈值', type: 'number', step: '0.05', hint: '预计配音时长超过字幕窗口该倍数时触发优化/变速（建议 1.1）' },
      { key: 'DUBBING_BGM_PATH', label: '背景音乐路径', type: 'text', hint: '可选：本地 BGM 文件绝对路径，留空无背景声', full: true },
      { key: 'DUBBING_BGM_VOLUME', label: 'BGM 音量', type: 'range', min: 0, max: 1, step: 0.05, hint: '相对配音音量，建议 0.3（氛围垫底）' },
    ],
  },
  {
    id: 'video',
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'PENDING_SCAN_INTERVAL_SECONDS', label: '待处理扫描间隔（秒）', type: 'number' },
      { key: 'STUCK_TASK_CHECK_INTERVAL_SECONDS', label: '卡住任务检查间隔（秒）', type: 'number' },
    ],
  },
  {
    id: 'notify',
