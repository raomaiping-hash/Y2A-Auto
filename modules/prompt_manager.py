...
_SUBTITLE_SHARED_RULES = (
    ...
    "对不完整短语、半句或续句，可在本条范围内补成一个自然短句，"
    "但不得改变原意、不得借用或融合相邻条目的信息。"
)
...
_SUBTITLE_STRICT_SHARED_RULES = (
    ...
    "若源文本是不完整短语、半句或续句，可在本条范围内尽量补成自然短句，不要硬留半截。"
)
...
_SUBTITLE_ZH_BUILTIN_BEHAVIOR = (
    "你是专业视频字幕翻译，把 texts 每一项翻译成自然、地道的简体中文，"
    "像字幕对白那样流畅，不要逐字直译，避免机翻腔。\n"
    "要求：\n"
    "- 用语口语化、自然通顺；\n"
    "- 每条译文尽量简短，多用短句；\n"
    "- 不输出任何标点符号：逗号、句号、问号、感叹号、分号、引号、省略号、括号等一律去掉；\n"
    "- 若一个条目包含多个短句/分句，请在**每个语义完整的短句之间用换行符 \\n 分隔**，"
    "每一行是一条完整、独立的短句（像 Netflix 单句字幕）；\n"
    "- 绝对不要把一句话拆成两行，也不要输出缺主谓的残缺半句——"
    "若某句语义不完整，请在本条目内补全或与紧邻短句合并成一个完整短句；\n"
    "- 对不完整短语或半句，在本条范围内补成一个通顺的短句（不跨条目、不改变原意、不添油加醋）；\n"
    "- **数字精确保留**：小数、分数、数字（如 16.5、10、20%）必须原样保留，不得四舍五入、改动或写成另一数字；\n"
    "- **禁止重复结巴**：不得连续重复同一个词或短语（如“它也可能它也可能”），同一意思只表述一次；\n"
    "- **不得截断**：必须译出完整句子，禁止半途截断或漏译结尾；\n"
    "- **相邻重复合并**：若相邻条目语义重复，只在其一处表述，不要重复翻译同一意思；\n"
    "- 数字、代码、URL、占位符和无公认译法的专有名词可保留；其余内容全部译成简体中文。"
)

_SUBTITLE_DEFAULT_BUILTIN_BEHAVIOR = (
    "你是专业视频字幕翻译，把 texts 每一项翻译成自然、地道的{target_language_name}，"
    "像字幕对白那样流畅，不要逐字直译，避免机翻腔。\n"
    "要求：\n"
    "- 用语口语化、自然通顺；\n"
    "- 每条译文尽量简短，单条不超过一整句话，多用短句；\n"
    "- 不输出任何标点符号：逗号、句号、问号、感叹号、分号、引号、省略号、括号等一律去掉，"
    "用空格或断行替代停顿；\n"
    "- 对不完整短语或半句，在本条范围内补成一个通顺的短句（不跨条目、不改变原意）；\n"
    "- 数字、代码、URL、占位符和无公认译名的专有名词可保留；其余内容全部翻译。"
)

# ---------- 字幕翻译严格补救 Prompt ----------
_SUBTITLE_STRICT_ZH_BUILTIN_BEHAVIOR = (
    "你是字幕翻译器（严格模式）。把 texts 每一项完整翻译成自然简体中文，"
    "普通句子和说明文字不得整句保留原文。译文简短、口语化，不输出任何标点符号，"
    "对不完整短语在本条范围内补成通顺短句。仅保留数字、代码、URL、占位符和必要专有名词。"
)

_SUBTITLE_STRICT_DEFAULT_BUILTIN_BEHAVIOR = (
    "你是字幕翻译器（严格模式）。把 texts 每一项完整翻译成{target_language_name}，"
    "除数字、代码、URL、占位符和专有名词外不要保留原文。译文简短、口语化，不输出任何标点符号，"
    "对不完整短语在本条范围内补成通顺短句。"
)
...
# ---------------------------------------------------------------------------
# 术语表/主题摘要 专用 prompt（VideoLingo step4_1 移植，非翻译型、独立于 Prompt 中心用户覆盖）
# ---------------------------------------------------------------------------

def get_glossary_prompt(target_language: str = "zh") -> str:
    """构建术语表 + 主题摘要提取的 system prompt。

    翻译前一次性调用：抽取全片主题摘要（1-2 句）与关键术语统一译法，
    用于后续翻译注入，保证术语前后一致、语境连贯。
    """
    return (
        "你是视频翻译的术语顾问和主题概括专家，精通源语言理解与"
        f"{target_language}表达优化。\n"
        "对下列视频字幕文本：\n"
        "1. 请用两句话概括视频主题（第一句讲主旨，第二句讲关键点）；\n"
        "2. 提取视频中反复出现或容易被误译的专业术语、人名、机构名、品牌名、"
        "专有名词等，给出统一的"
        f"{target_language}译法；\n"
        "3. 每个术语附一句简要解释（note），便于翻译时保持一致性。\n"
        "只提取真正影响翻译一致性的术语，少于 15 个。\n"
        "只返回 JSON：{\"summary\":\"两句话主题\","
        f"\"terms\":[{{\"src\":\"源语术语\",\"tgt\":\"{target_language}译法\",\"note\":\"简短解释\"}}]}}。"
    )


# ---------------------------------------------------------------------------
# 字幕超窗句修剪 prompt（借鉴 VideoLingo check_len_then_trim / get_subtitle_trim_prompt）
# ---------------------------------------------------------------------------
...
def get_subtitle_trim_prompt(text: str, duration_s: float) -> str:
    ...
    return (
        "你是专业字幕编辑。下面的字幕太长了，无法在给它限定的时间内自然读完，"
        "请把它精简成能在规定时长内清晰读完的短句（约在 %.1f 秒内、约 %d 字以内）。\n"
        "要求：\n"
        "- 只删填充词、冗余修饰语、可省代词/连接词，保留核心内容和原意；\n"
        "- 不要改变事实、数字、专有名词；\n"
        "- 结果是一句通顺、完整、更短的中文，不要分句、不要加解释；\n"
        "- 只返回 JSON：{\"result\":\"精简后的字幕\"}。\n"
        "待精简字幕：[%s]"
    ) % (float(duration_s), max_chars, text)
